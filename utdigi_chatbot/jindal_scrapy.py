# scrapy_jsom.py — JSOM-only UTD Crawler
...
# ✅ MongoDB connection (new collection for JSOM data)
client = MongoClient("mongodb://localhost:27017/")
collection = client["utdallas_db_mini"]["jsom_data"]

def clean_text(texts):
    return list(set(t.strip() for t in texts if t.strip()))

class JSOMSpider(scrapy.Spider):
    name = "utd_jsom_only"
    allowed_domains = ["jindal.utdallas.edu"]
    start_urls = ["https://jindal.utdallas.edu/"]
    visited_urls = set()
    ...
    def parse(self, response):
        try:
            ...
            # ✅ Follow internal JSOM links only
            for href in response.css("a::attr(href)").getall():
                full_url = response.urljoin(href).split("#")[0].rstrip("/")
                parsed = urlparse(full_url)

                if (
                    "jindal.utdallas.edu" in parsed.netloc
